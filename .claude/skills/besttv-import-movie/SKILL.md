...
Админ    admin@besttv.mn / Mongol.,123
         ⚠️ Нууц үг СОЛИГДДОГ — 401 ирвэл хэрэглэгчээс асуу
           (2026-08-25: Admin@12345 → Mongol.,123)
...
### ⚠️⚠️ ӨГӨГДМӨЛ — АСУУХГҮЙ

XPLAY-ийн контент нь БАРАГ ҮРГЭЛЖ ижил тул хэрэглэгч өөрөөр
заагаагүй бол ЭДГЭЭРИЙГ шууд хэрэглэ:

```
genreId  cmsk1ahn400007s3gmsj41tej   (Богино болон С-drama)
language MN                          (монголоор шивэгдсэн)
```

⚠️ `info.txt`-ийн жанр (Драм, Романтик…) нь BestTV-ийн ангилалтай
ТААРАХГҮЙ — үл тоомсорло.

⚠️ ЗӨВХӨН дараах үед асуу:
  · Кино нь тодорхой МОНГОЛ бүтээл (эх сурвалж нь монгол студи)
  · Ангийн урт 20+ минут (богино драм биш → Шилдэг кино байж болно)
  · Хэрэглэгч өөрөө «энэ бол өөр жанр» гэж дурдсан
...
⚠️⚠️ **`complete`-ийн `parts` нь ТОМ үсгээр**: `{ "PartNumber": n,
  "ETag": "..." }`. Жижиг үсгээр (`partNumber`/`etag`) илгээвэл S3 руу
  хоосон XML явж **500 «The XML you provided was not well formed»**
  гэнэ — алдааны мессеж нь шалтгааныг ОГТ заахгүй тул удаан төөрүүлнэ.

⚠️ `multipart/urls` нь `{ "urls": [{partNumber, url}] }` гэж
  БҮРХҮҮЛТЭЙ буцаана (шууд массив биш).
...
⚠️⚠️ **КИРИЛЛ ӨГӨГДЛИЙГ `curl`-ЭЭР БҮҮ ИЛГЭЭ** — Windows дээр
   гажина. БОДИТ АЛДАА (2026-08-25): `curl -d '{"year":2027,...}'`
   илгээхэд DB-д **2026** гэж бичигдэж, `metaTitle` нь
   `??????? ??????` болж эвдэрсэн.

   Засах/шинэчлэхдээ ЗААВАЛ Python ашигла:
   ```python
   m.api(f'/admin/titles/{tid}', {'metaTitle': mt}, method='PATCH')
   ```

⚠️ **`isActive` нь `@default(true)`** — Prisma схемд. Хэрэв
   `create`-д дамжуулсан утга ямар нэг шалтгаанаар алдагдвал кино
   ШУУД нийтлэгдэнэ. Импортын ДАРАА заавал шалга:
   ```
   select "isActive", year, "metaTitle" from "Title" where slug='…'
   ```
