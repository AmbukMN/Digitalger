...
## 21. Алдааны мессеж — БҮГД монгол байх ёстой

⚠️⚠️ Backend-ийн exception мессеж нь frontend-д **шууд toast-оор**
харагддаг (`api.ts` → `body?.message`). Англи бичиг гарвал хэрэглэгч
юу болсныг ойлгодоггүй.

Илэрсэн 2 газар:
| Guard | Өмнө | Одоо |
|---|---|---|
| `JwtAuthGuard` | `Authentication required` | `Нэвтэрч орно уу` |
| `RolesGuard` | `Forbidden resource` (Nest default) | `Танд энэ үйлдэл хийх эрх байхгүй` |

⚠️ `RolesGuard` нь `return false` хийдэг байсан тул Nest-ийн **англи
default** гардаг байв. `ForbiddenException` шидэх ёстой.

⚠️ Frontend талд mapping хийж болохгүй — шинэ endpoint нэмэх бүрд
мартагдана. Орчуулга нь ЭХ СУРВАЛЖ дээр байна.

---

## 22. Нэвтрэлт шаардсан товч — `loginUrlWithIntent` ЗААВАЛ

⚠️ Товч нь нэвтрэлт шалгахгүй бол API нь 401 буцааж хэрэглэгч
ойлгомжгүй алдаа хардаг.

Загвар:
```ts
if (!user) {
  router.push(loginUrlWithIntent({ type: 'buy-plan', planId, method: 'bank' }));
  return;
}
```

`AuthIntent.method` = `'wallet' | 'qpay' | 'bank'` — нэвтэрсний дараа
`consumeAuthIntent()` нь ТУХАЙН үйлдлийг автоматаар үргэлжлүүлнэ.

⚠️ Шинэ төлбөрийн арга нэмэх бүрд `method` union-д нэмж,
intent боловсруулагчид салаа нэмэх ёстой.

---

## 23. Төлбөртэй контент — зочныг ЛОГИН руу шидэхгүй

⚠️⚠️ `usePlayGuard` нь `isPremium && !user` үед шууд `/login` руу
шиддэг байв. Хэрэглэгч **үнэ, багцын сонголтыг хараагүй** атлаа
бүртгүүлэхийг шаардуулна — ихэнх нь тэндээ буцна.
