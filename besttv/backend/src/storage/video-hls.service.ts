 ...
 *
 * ⚠️⚠️ ФОРМАТ: ffmpeg БҮРЭН re-encode хийдэг (libx264 + aac) тул ОРОЛТЫН
 * формат бараг хамаагүй — mp4, mkv, avi, mov, wmv, flv, ts бүгд болно.
 * MKV доторх HEVC/H.265, VP9 ч H.264 болж хөрвөнө (браузер HEVC тоглуулдаггүй
 * тул энэ нь ЗААВАЛ). Гаралт нь ҮРГЭЛЖ HLS (.m3u8 + .ts) — player зөвхөн
 * түүнийг л мэднэ.
 *
 * ⚠️ Хуучин тайлбар «-c copy, re-encode ХИЙХГҮЙ» гэж бичсэн нь ХУУЧИРСАН —
 * ABR (олон чанар) нэмэгдсэнээс хойш libx264-ээр дахин encode хийдэг.
 *
  ...
  private probeHasAudio(filePath: string): Promise<boolean> {
    return new Promise((resolve) => {
      ffmpeg.ffprobe(filePath, (err, data) => {
        ...
        /**
         * ⚠️⚠️ ОЛОН АУДИОТОЙ ФАЙЛ (MKV-д ТҮГЭЭМЭЛ).
         *
         * `-map a:0?` нь ЭХНИЙ аудио урсгалыг л авна. MKV нь ихэвчлэн
         * олон дуутай байдаг (англи + монгол дубль). Хэрэв эхнийх нь
         * англи бол хэрэглэгч АНГЛИ дуутай кино үзнэ — админ юу
         * болсныг ойлгохгүй, «дуу нь буруу» гэсэн гомдол ирнэ.
         *
         * Оношлоход хэцүү тул ЗААВАЛ логлоно. (Тодорхой урсгал сонгох
         * боломж хэрэгтэй бол админд хэл сонгуулах ёстой — одоохондоо
         * сануулга хангалттай, ихэнх файл нэг дуутай.)
         */
        if (audio.length > 1) {
          const langs = audio
            .map((s, i) => `${i}:${(s.tags as { language?: string })?.language ?? '?'}`)
            .join(' ');
          this.logger.warn(
            `⚠️ Видео ${audio.length} аудио урсгалтай — ЭХНИЙХ нь сонгогдоно (${langs}). ` +
              `Буруу хэл орвол эх файлаас хэрэгтэй дууг нь үлдээж дахин оруулна уу.`,
          );
        }

        resolve(audio.length > 0);
      });
    });
  }
