...
async function main() {
   ...
   */
  /**
   * ⚠️⚠️ 3 — 10 БИШ. БОДИТООР ХЭМЖСЭН:
   *   1 файл  → 7.8 MB/s
   *   10 файл → 7 урсгал × 0.8 = ~5.6 MB/s (УДААН!)
   * Таны сүлжээний дээд хязгаар ~8 MB/s тул олон урсгал нь тэр хэмжээг
   * ХУВААХААС гадна TCP өрсөлдөөн, R2-ийн хариу хүлээлт нэмэгдэж
   * НИЙТ хурдыг БУУРУУЛНА. 3 нь latency-г нөхөх хэрэгтэй доод тоо.
   */
  ...
  async function uploadOne(item: (typeof list)[0]) {
    ...
    try {
      /**
       * ⚠️ ЭХЛЭХЭД `UPLOADED` тэмдэглэнэ — админд явц ХАРАГДАНА.
       *
       * Өмнө нь зөвхөн байршуулалт ДУУССАНЫ дараа DB-д бичдэг байсан
       * тул админ панельд "Видео ороогүй" гэж харагдсаар, хэрэглэгч
       * "ажиллаж байна уу?" гэж эргэлздэг байв (бодит гомдол).
       *
       * ⚠️ `UPLOADED` нь `PROCESSING`-ЭЭС ӨӨР: worker үүнийг авахгүй
       * (queue-д ороогүй), зөвхөн "байршиж байна" гэсэн тэмдэглэгээ.
       * ⚠️ Алдаа гарвал үлдэх нь ЗӨВ — дараагийн ажиллалт `videoKey`
       * байхгүйг хараад дахин оролдоно.
       */
      await prisma.title
        .update({ where: { id: item.title.id }, data: { streamStatus: 'UPLOADED' } })
        .catch(() => null);
