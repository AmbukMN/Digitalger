  ...
  return (
    <main className="min-h-screen bg-background px-4 pb-16 pt-28 md:px-8">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h1 className="mt-4 text-3xl font-black tracking-tight text-white md:text-4xl">Мэдээ, нийтлэл</h1>
      </div>
      ...
      <div className="mx-auto mt-10 grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {data?.items.map((p) => (
          <Link
            key={p.id}
            href={`/blog/${p.slug}`}
            className="group overflow-hidden rounded-xl bg-white/5 transition-colors hover:bg-white/8"
          >
            ...
            <div className="p-4">
              <h2 className="line-clamp-2 font-semibold text-white/90">{p.title}</h2>
              {p.excerpt && <p className="mt-1.5 line-clamp-2 text-sm text-white/50">{p.excerpt}</p>}
      ...
      {!isLoading && data?.items.length === 0 && (
        <p className="mt-16 text-center text-white/40">Одоогоор нийтлэл байхгvй байна</p>
      )}
          ...
          {Array.from({ length: data.totalPages }).map((_, i) => (
            <button
              ...
              className={cn(
                ...
                page === i + 1 ? 'bg-primary text-white' : 'bg-white/8 text-white/60 hover:bg-white/12',
              )}
            >
              {i + 1}
            </button>
