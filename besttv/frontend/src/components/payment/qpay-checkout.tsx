...
function BankAppButton({
  ...
  return (
    <a
      ...
      className="flex flex-col items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 p-2.5 transition-all hover:bg-white/10 active:scale-95"
    >
      ...
      />
      <span className="line-clamp-2 w-full text-center text-[9px] font-medium leading-tight text-white/60">
        {name}
      </span>
    </a>
  );
...
function StepsDesktop() {
  ...
  return (
    <div className="hidden space-y-2 rounded-xl border border-white/8 bg-white/4 px-4 py-3 sm:block">
      <p className="mb-1 text-xs font-semibold text-white">3 алхамаар амархан төлнө</p>
      {steps.map((s, i) => (
        <div key={i} className="flex items-start gap-2.5">
          ...
          <span className="text-xs leading-relaxed text-white/75">{s}</span>
        </div>
      ))}
      <p className="mt-1 border-t border-white/8 pt-1 text-[10px] leading-relaxed text-white/40">
...
function StepsMobile() {
  return (
    <div className="rounded-xl border border-primary/25 bg-white/5 px-4 py-3 sm:hidden">
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="fixed left-1/2 top-1/2 z-100 flex max-h-[90dvh] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#141518] shadow-2xl"
      >
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between border-b border-white/8 px-5 py-3.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold leading-tight text-white">QPay-ээр төлбөр хийх</p>
        ...
        <div className="flex-1 overflow-y-auto">
          {paid ? (
            <div className="flex flex-col items-center gap-3 px-5 py-10">
              ...
              <p className="text-lg font-bold text-white">Төлбөр амжилттай</p>
            ...
            <div className="space-y-4 p-5">
              {invoice.amount != null && (
                <div className="flex items-baseline justify-between rounded-xl border border-white/8 bg-white/4 px-4 py-3">
                  <span className="text-sm text-white/55">Төлөх дүн</span>
                  <span className="text-xl font-black text-white">{formatPrice(invoice.amount)}</span>
                </div>
              )}
              ...
              {banks.length > 0 && !expired && (
                <div className="sm:hidden">
                  <p className="mb-1 text-sm font-semibold text-white">Банкны аппаа сонгоно уу</p>
              ...
              {expired && (
                <div className="py-8 text-center">
                  <p className="text-sm text-white/60">QR-ийн хугацаа дууслаа</p>
                  <button
                    onClick={onClose}
                    className="mt-3 rounded-lg bg-primary px-5 py-2 text-sm font-semibold text-white hover:brightness-110"
                  >
                    Хаах
                  </button>
                </div>
              ...
              {!qrSrc && banks.length === 0 && !expired && (
                <div className="flex flex-col items-center gap-2 py-6 text-center">
                  ...
                  <p className="text-sm text-white/55">Төлбөрийн мэдээлэл ачаалж байна…</p>
                </div>
              )}
        ...
        {!paid && !expired && (
          <div className="shrink-0 space-y-3 border-t border-white/8 bg-[#141518] px-5 py-4">
            <StepsDesktop />
            <StepsMobile />
            ...
            <button
              ...
              disabled={checking}
              className="flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-primary text-sm font-semibold text-white shadow-md transition-all hover:brightness-110 disabled:opacity-60"
            >
              {checking ? (
