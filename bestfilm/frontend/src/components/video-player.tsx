  ...
  useEffect(() => {
    return () => {
      ...
      try {
        ...
        /**
         * ⚠️⚠️ `pause()` ГАНЦААРАА ХҮРЭЛЦЭЭГҮЙ (бодит хэмжилт).
         *
         * 08-26-ны засварын дараа алдаа 2.07→1.53/цаг болж БУУРСАН ч
         * БҮРМӨСӨН арилаагүй (12 цагт 19 удаа, бүгд iPhone, бүгд
         * `/movie/*` = трейлерийн модал).
         *
         * ШАЛТГААН: `pause()` нь ВИДЕОГ зогсоодог ч slider-ийн DOM нь
         * React unmount хүртэл АМЬД хэвээр. WebKit нь `pointercancel`
         * /`touchend`-ийг ТЭР ЗАЙД илгээвэл slider дахин
         * `this.$props.disabled()` рүү хандаж, props нь аль хэдийн
         * задарсан тул `t is not a function` болж унана.
         *
         * ЗАСВАР: устгахын өмнө plеерийн DOM-д `pointer-events: none`
         * тавина — browser нь ЦААШИД pointer event ИЛГЭЭХГҮЙ болно.
         * Энэ нь Vidstack-ийн дотоод кодыг залруулах гэж оролдохгүйгээр
         * (сангийн алдаа, бидний код биш) шалтгааныг таслана.
         *
         * ⚠️ Хэрэглэгчид ХАРАГДАХГҮЙ: энэ агшинд компонент устаж
         * байгаа тул харилцан үйлчлэл аль хэдийн дууссан.
         */
        /* ⚠️ `p.el` нь Vidstack-ийн дотоод талбар — хувилбар солигдоход
           алга болж болзошгүй тул DOM-оос ШУУД хайна (`media-player`
           бол вэб компонентын тэмдэглэгээ, тогтвортой). */
        const el =
          (p as unknown as { el?: HTMLElement | null }).el ??
          document.querySelector<HTMLElement>('media-player');
